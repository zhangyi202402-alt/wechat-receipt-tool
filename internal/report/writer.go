...
func (w *Writer) WriteDateSummary(path, dateStr string, stores []StoreReport, totalRaw, totalDedup int, excelWritten, excelSkipped bool) error {
	...
	fmt.Fprintf(&b, "门店数: %d | 总图片: %d | 识别记录: %d | 合并去重后: %d | 待核对: %d\n",
		len(stores), sumImages(stores), totalRaw, totalDedup, sumNeedsReview(stores))
	if excelSkipped {
		b.WriteString("Excel: 已跳过（文件已存在，使用 --force 覆盖）\n")
	} else if excelWritten {
		b.WriteString("Excel: 已生成（所有门店合并）\n")
	}
	b.WriteString("说明: 待核对行请打开 Excel「待核对原因」列并对照「原图片段」确认\n")
	b.WriteString("===\n")
	for _, sr := range stores {
		...
		fmt.Fprintf(&b, "图片数: %d | 识别: %d | 去重: %d | 待核对: %d\n",
			sr.ImageCount, sr.RawRecords, sr.DedupRecords, sr.NeedsReview)
		for _, img := range sr.Images {
			...
			if img.NeedsReview > 0 {
				line += fmt.Sprintf(" | 待核对 %d 条", img.NeedsReview)
			}
			if len(img.LowConfidence) > 0 {
				line += fmt.Sprintf(" | 低置信度 %d 条", len(img.LowConfidence))
			}
			fmt.Fprintf(&b, "%s\n", line)
			for _, item := range img.ReviewItems {
				fmt.Fprintf(&b, "    - %s\n", item)
			}
		}
...
func sumNeedsReview(stores []StoreReport) int {
	n := 0
	for _, s := range stores {
		n += s.NeedsReview
	}
