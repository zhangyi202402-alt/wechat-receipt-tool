...
按日期/门店目录组织微信「零钱明细」截图，OCR 识别账单交易（二维码收款、转账、红包、商户消费等），生成 Excel 台账。

## 功能

- `init`：创建 `{data}/{日期}/{门店}/` 目录（门店列表可配置）
- `process`：扫描截图 → OCR → 生成 `收款记录.xlsx` 与 `处理报告.txt`
- 待核对行：带置信度、可读「待核对原因」、原图交易带片段（`data/{日期}/review/{门店}/`）
...
|----------|------|
| 门店 | 门店子目录名称 |
| 类型 | 二维码收款 / 转账收入 / 转账支出 / 红包 / 退款 / 提现 / 商户消费 / 其他 |
| 收支 | 收入 / 支出 |
| 对方/摘要 | 截图标题解析出的对方或摘要 |
| 金额（元） | 有符号：收入为正、支出为负 |
| 状态 | 正常 / 待核对 |
| 置信度 | 综合 OCR 与字段完整度 |
| 待核对原因 | 如「缺日期」「金额列与整图冲突」；正常行为空 |
| 原图片段 | 待核对行直接嵌入的交易带缩略图 |

待核对行请看「待核对原因」并对照「原图片段」确认（表内不设人工填写列）。
...
ocr:
  ...
  amount_column_start: 0.75    # 从 75% 宽度起裁剪，右侧 25% 为金额列
